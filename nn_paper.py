...
def main(timeout):
    ...
    starttime = time.time()
    org_nnrb_url = 'http://www.nnrb.com.cn/'
    ...
    headers = { 'User-Agent' : user_agent , "Cache-Control" : "no-cache" , "Connection" : "keep-alive", "Referer": org_nnrb_url}

    #o_nnrb_url = 'http://paper.people.com.cn/nnrb/html/2014-06/22/node_5.htm'
    req = urllib2.Request(str(org_nnrb_url), headers=headers)
    try:
        fin = urllib2.urlopen(req,timeout=timeout).read()
    except:
        print '[-] wget error' , org_nnrb_url 
        #raw_input('check')
        sys.exit('check your network, or if the websize is down.')
        #time.sleep(1)

    new_re = re.compile(r'URL=(.*?)"',re.DOTALL).findall(fin)
    o_nnrb_url = 'http://www.nnrb.com.cn/' + new_re[0]
    #http://www.nnrb.com.cn/html/2014-06/22/node_102.htm

    print 'o_nnrb_url:', o_nnrb_url
    #raw_input()
    ...
    user_agent = random.choice(user_agents_list).replace('\r\n','').replace('\r','').replace('\n','')
    headers = { 'User-Agent' : user_agent , "Cache-Control" : "no-cache" , "Connection" : "keep-alive", "Referer": o_nnrb_url}

    #o_nnrb_url = 'http://paper.people.com.cn/nnrb/html/2014-06/20/node_1921.htm'  #etc...
    req = urllib2.Request(str(o_nnrb_url), headers=headers)
    try:
        fin = urllib2.urlopen(req,timeout=timeout).read()
    except:
        print '[-] wget error' , o_nnrb_url 
        #raw_input('check')
        sys.exit('check your network, or if the websize is down.')
        #time.sleep(1)
    ...
    #http://www.nnrb.com.cn/html/2014-06/22/node_7.htm #for re_

    #o_day_url = 'http://www.nnrb.com.cn/html/2014-06/22/'
    o_day_url = 'http://www.nnrb.com.cn/html/' + today_new_re[0][0] + '-' + today_new_re[0][1] + '/' + today_new_re[0][2] + '/'
    print 'o_day_url:', o_day_url
    #raw_input()

    nnrb_list = []
    nnrb_list.append(o_nnrb_url)
    for n_link in new_re:
        #print n_link
        if './' in n_link:continue
        nnrb_list.append(o_day_url + n_link)
        #nnrb_list.append(o_day_url + 'nbs' + n_link)
    ...
    for n_link in nnrb_list:
        ...
        try:
            ...
            #raw_input('check')
            continue
        new_re_title = re.compile('class=px12><div align="center">(.*?)</div>',re.DOTALL).findall(fin)
        the_title = new_re_title[0].decode('utf-8').encode('gbk').replace('\\','_').replace('*','_').replace('\/','_').replace('/','_').replace(':','_').replace('\*','_').replace('\|','_').replace('|','_').replace('?','_').replace('\?','_').replace('\"','_').replace('"','_').replace('\<','_').replace(' ','').replace('\t','').replace('\r\n','').replace('\r','').replace('\n','').replace('·','.')
        ...
        for m_link in new_re_link2:
            ...
            new_re2 = re.compile(r';line-height:30px;">(.*?)</strong>.+?line-height:30px;">(.*?)</span>.+?<founder-content>(.*?)</founder-content>',re.DOTALL).findall(fin)
